...
load.libraries <- c('dplyr','ggplot2','readr','mice','Hmisc','missForest','xgboost',
                    'dummies','randomForest','Metrics','rpart')
...
prin_comp_train <- prcomp(total_df[1:test_rownum,],scale. = T)
std_dev_train <- prin_comp_train$sdev
pr_var_train <- prin_comp_train$sdev^2
#Proportion of variance explained by a Principal component
prop_varex_train <- pr_var_train/sum(pr_var_train)
#Scree plot
plot(prop_varex_train,xlab="Princpial Compenent",ylab = "Proportion of variance Explained",type = "b")
#Cumulative Scree plot
plot(cumsum(prop_varex_train),xlab = "Principal Component",
     ylab="Cumulative proportion of variance explained", type="b")
#200 PR comps explain more than 99% of the variance

train_new <- data.frame(prin_comp_train$x)[1:200]
...
test_new <- predict(prin_comp_train,total_df[((test_rownum+1):dim(total_df)[1]),])
test_new <- as.data.frame(test_new[,1:200])
...
write.csv(Results,"PCALinearRegSubmission.csv",row.names = FALSE)
