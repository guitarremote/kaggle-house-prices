...
omit_fields <- c()
for(i in 1:dim(dummy_factor_df)[2]){
  omit_fields <- c(omit_fields,paste0(names(dummy_factor_df)[i],"_",names(table(dummy_factor_df[,i]))[length(names(table(dummy_factor_df[,i])))]))
}
...
imp_features <- c("LotArea","OverallQual","OverallCond","BsmtExposure","BsmtFinSF1","BsmtFinSF2","BsmtUnfSF","X1stFlrSF",
                  "X2ndFlrSF","LowQualFinSF","BsmtFullBath","FullBath","HalfBath","KitchenQual","Fireplaces",
                  "GarageArea","GarageQual","WoodDeckSF","ScreenPorch","MSZoning_C (all)","Utilities_AllPub",
                  "LotConfig_CulDSac","Neighborhood_BrDale","Neighborhood_CollgCr","Neighborhood_Edwards",
                  "Neighborhood_IDOTRR","Neighborhood_MeadowV","Neighborhood_Mitchel","Neighborhood_NAmes","Neighborhood_NWAmes",
                  "Neighborhood_OldTown","Neighborhood_Sawyer","Condition2_PosA","RoofStyle_Gable","RoofStyle_Gambrel",
                  "RoofStyle_Hip","RoofMatl_ClyTile","RoofMatl_WdShake","Exterior1st_BrkComm","Exterior1st_BrkFace",
                  "Foundation_PConc","Foundation_Stone","Heating_Grav","HeatingQC_Ex","CentralAir_N","Functional_Maj1",
                  "Functional_Maj2","Functional_Min2","Functional_Mod","Functional_Sev","SaleType_ConLD","SaleType_New",
                  "YRBuilt_Range_1951-1960","YRBuilt_Range_1961-1970","YRBuilt_Range_1971-1980","YRBuilt_Range_1981-1990",
                  "YRBuilt_Range_1991-2000","YRBuilt_Range_2001-2005","YRBuilt_Range_2006-2010")

train_new <- total_df[1:test_rownum,imp_features]
test_new <- total_df[((test_rownum+1):dim(total_df)[1]),imp_features] 

# Simple Linear Model
fold <- rep_len(1:10,nrow(train_new))
fold <- sample(fold,nrow(train_new))
#10 fold cross validation
err <- c()
rsquared <- c()
for(i in 1:10){
  idx <- which(fold==i)
  tTrain <- train_new[-idx,]
  tTest <- train_new[idx,]
  modellm <- lm(log(SalePrice[-idx])~.,tTrain)
  prediction <- predict(modellm,tTest)
  print("Ok")
  err <- c(err,rmse(log(SalePrice[idx]),prediction))
  rsquared <- c(rsquared,summary(modellm)$r.squared) 
}

mean(err)
mean(rsquared)

linearm <- lm(log(SalePrice)~.,train_new)
testResults <- predict(linearm,test_new)
Results <- data.frame(Id=as.integer(names(testResults)), SalePrice= exp(testResults))

write.csv(Results,"LinearRegSubmission.csv",row.names = FALSE)
...
for (i in 1:(dim(train_new)[2])){
  train_new[,i] <- as.numeric(train_new[,i])
}
for (i in 1:(dim(test_new)[2])){
  test_new[,i] <- as.numeric(test_new[,i])
}
...
for(i in 1:10){
  idx <- which(fold==i)
  tTrain <- xgb.DMatrix(as.matrix(train_new[-idx,]),label=log(SalePrice[-idx]))
  tTest <- xgb.DMatrix(as.matrix(test_new[idx,]))
  best_n_rounds <-1000
  modelgb<- xgb.train(xgb_params,tTrain,nrounds = as.integer(best_n_rounds))
  prediction <- predict(modelgb,tTest)
  print("OK")
  err_xg <- c(err_xg,rmse(log(SalePrice[idx]),prediction))
}

mean(err_xg)

dtrain <- xgb.DMatrix(as.matrix(train_new),label=log(SalePrice))
dtest <- xgb.DMatrix(as.matrix(test_new))
best_n_rounds <-1000

gb_dt<- xgb.train(xgb_params,dtrain,nrounds = as.integer(best_n_rounds))
Resultsgb <- predict(gb_dt,dtest)
Resultsxgboost <- data.frame(Id=1:1459, SalePrice= exp(Resultsgb))
write.csv(Results,"XGBoostSubmission.csv",row.names = FALSE)
